...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
   ...
   protected int height(BSTNode<T> n) {
      if (n.isEmpty()) {
         return -1;
      } else {
   ...
   private void preOrder(BSTNode<T> node, ArrayList<T> aux) {
      //R,E,D
      if (!node.isEmpty()) {
   ...
   private void order(BSTNode<T> node, ArrayList<T> aux) {
      //E,R,D
      if (!node.isEmpty()) {
   ...
   private void postOrder(BSTNode<T> node, ArrayList<T> aux) {
      //E,D,R
      if (!node.isEmpty()) {
